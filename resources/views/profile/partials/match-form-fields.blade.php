...
<div x-data="matchForm({
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        <label class="flex cursor-pointer items-center gap-3 rounded border border-gray-200 p-3">
            <input type="radio"
                   name="status"
                   value="{{ MatchGame::STATUS_UPCOMING }}"
                   x-model="status"
                   class="border-gray-300 text-yellow-500">
            <span>
        ...
        <label class="flex cursor-pointer items-center gap-3 rounded border border-gray-200 p-3">
            <input type="radio"
                   name="status"
                   value="{{ MatchGame::STATUS_FINISHED }}"
                   x-model="status"
                   class="border-gray-300 text-yellow-500">
            <span>
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <label class="relative block">
            ...
            <input name="opponent_name"
                   ...
                   @input.debounce.300ms="loadOpponents($event.target.value)"
                   @focus="loadOpponents($event.target.value)">

            <div x-show="opponents.length"
                 x-cloak
                 class="absolute z-30 mt-1 w-full overflow-hidden rounded border border-gray-200 bg-white shadow-lg">
        ...
        <label class="block">
            ...
            <input name="match_date"
                   type="datetime-local"
                   required
                   :min="status === '{{ MatchGame::STATUS_UPCOMING }}' ? '{{ now()->format('Y-m-d\T00:00') }}' : null"
        ...
        <label class="relative block">
            ...
            <input name="location"
                   ...
                   class="mt-1 w-full rounded border-gray-300"
                   @input.debounce.300ms="loadLocations($event.target.value)"
                   @focus="loadLocations($event.target.value)">

            <div x-show="locations.length"
                 x-cloak
                 class="absolute z-30 mt-1 w-full overflow-hidden rounded border border-gray-200 bg-white shadow-lg">
        ...
        <label class="block">
            ...
            <input type="time"
                   class="mt-1 w-full rounded border-gray-300"
                   @change="syncTime($event.target.value)">
        </label>
        ...
        <div class="grid gap-4 md:col-span-2 md:grid-cols-2" x-show="status === '{{ MatchGame::STATUS_FINISHED }}'">
            <label class="block">
                ...
                <input name="our_score"
                       ...
                       min="0"
                       max="999"
                       :disabled="status !== '{{ MatchGame::STATUS_FINISHED }}'"
