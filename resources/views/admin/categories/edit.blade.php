...
<div class="mb-6">
    ...
    <h1 class="mt-2 text-2xl font-bold text-zinc-100">{{ $category->name }}</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-5">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3"
                      class="mt-1 block w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400">{{ old('description', $category->description) }}</textarea>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" class="ml-auto" onsubmit="return confirm('Usunąć kategorię „{{ $category->name }}"?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="text-sm text-red-400 hover:text-red-300 transition-colors">Usuń</button>
            </form>
        </div>
    </form>
</div>
